...
void __weak
test_write_32_64(void)
{
	unsigned word __attribute__ ((aligned(16))) = 0x80000001;
	unsigned *addr = (unsigned*) 0x4000000UL;
	
#if 0
	*(volatile unsigned*) addr = word;
	*(volatile unsigned long*) (addr+2) = word;
#endif
	writel(word, addr);
	writeq(word, addr+4); 
