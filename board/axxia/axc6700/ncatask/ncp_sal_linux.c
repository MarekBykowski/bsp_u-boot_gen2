...
#if (defined NCP_SAL_HOSTMEM_NVM) || (defined NCP_SAL_SHM_NVM)
{
    ...
    ncp_uint32_t    reg;
    /* 
     * platforms for which NCP_SAL_HOSTMEM_NVM is defined will use 
     * a reserved region of host memory for NVM. On the internal core
     * this will be ACP sysmem.  The address/size of the NVM region 
     * are specified  in the config image and programmed to driver 
     * virtual registers at config load time. Here we need to open 
     * the driver to read these values before we can map the NVM region.
     */

    ncp_dev_open(0, 0, &devHdl);

    /* get the NVM base address */
    ncp_dev_read32(devHdl, NCP_REGION_DRIVER_CFG, 
                           NCP_DOMAIN_LOCAL_NVM_BASE_ADDR,
                           &reg);
    if (reg == 0) {
        NCP_NVM_INIT_ERR(printf("NVM base address not defined!\n"));
        return NULL;
    }
    offset = (off_t) (((off_t) reg) << 20);

    /* get the NVM size */
    ncp_dev_read32(devHdl, NCP_REGION_DRIVER_CFG, 
                           NCP_DOMAIN_LOCAL_NVM_SIZE,
                           &reg);
    if (reg == 0) {
        NCP_NVM_INIT_ERR(printf("NVM size not defined!\n"));
        return NULL;
    }
    ncp_nvm_size = (reg << 20);

    devfd = (intptr_t) devHdl;

#ifdef NCP_SAL_SHM_NVM
    /* open shared memory resgion */
    fd = (shm_open("/ncpNvmState", (O_RDWR | O_CREAT), 0666 ) ) ;

    if  ( fd < 0)
    {
        NCP_NVM_INIT_ERR(perror("shm_open"));
        return NULL;
    }

    if ( 0 > ftruncate(fd, ncp_nvm_size) )
    {
        NCP_NVM_INIT_ERR(perror("ftruncate"));
        return NULL;
    };
...
#else 
    char *tmpDir            = NULL;
    char nvmFileName[256]   = {0};
    char null               = 0;

    /* on the external host we use a memory mapped file */
    /* 
     * get the path for the nvm.dat file. 
     */
    tmpDir = getenv("NCP_TMP_DIR");
    if (tmpDir != NULL)
    {
        strcpy(nvmFileName, tmpDir);
        strcat(nvmFileName, "/ncpNvmState.dat");
    }
    else
    {
        strcpy(nvmFileName, "/tmp/ncpNvmState.dat");
    }
    offset = (off_t) 0;

    /*
     * open the file for the memory-mapped I/O, creating it if it
     * does not exist. The open() does not seem to set write permissions
     * for group and others, so explicitly set the file permissions.
     * Grow the file size to be the max NVM size.
     */

    fd = open(nvmFileName, O_RDWR | O_CREAT , 0666);
    if (fd == -1)
    {
        NCP_NVM_INIT_ERR(printf("Could not open NVM file %s\n", nvmFileName));
        NCP_NVM_INIT_ERR(perror("open"));
        return NULL; 
    }

    if (chmod(nvmFileName, 0666) < 0)
    {
        /*
         * Generally this only fails if the file already exists
         * and was created by root, and this process is running
         * as non-root. Since in this case the file-permissions
         * have already been set, ignore the error.
         */
        if (errno != EPERM)
        {
            NCP_NVM_INIT_ERR(perror("chmod"));
            return NULL;
        }
    }

    /* set the file size */
    if (lseek(fd, ncp_nvm_size, SEEK_SET) < 0)
    {
        NCP_NVM_INIT_ERR(perror("lseek"));
        return NULL;
    }

    if (write(fd, (void *) &null, sizeof(char)) == -1)
    {
        NCP_NVM_INIT_ERR(perror("write"));
        return NULL;
    }
#endif

    /*
     * map the nvm.dat file to virtual address space at ncp_nvm_vaddr
     */
    pnvm_base = (char *)mmap((void *)ncp_nvm_vaddr, ncp_nvm_size, 
                                     (PROT_READ | PROT_WRITE), 
                                     MAP_SHARED, fd, offset);

    if (pnvm_base == (char *) MAP_FAILED)
    {
        NCP_NVM_INIT_ERR(perror("mmap"));
        return NULL;
    }

    if (pnvm_base != (char *) ncp_nvm_vaddr)
    {
        NCP_NVM_INIT_ERR(printf("mmap returned wrong addr 0x%s (expected 0x%s)\n", 
	    NCP_CONVERT_LONG_TO_HEX_STR((ncp_uint64_t)((unsigned long)pnvm_base)),
	    NCP_CONVERT_LONG_TO_HEX_STR((ncp_uint64_t)((unsigned long)ncp_nvm_vaddr))));
        return NULL;
    }
