    ...
    if (NULL == pProcess)
    {
             ...
             i++)
        {
            ...
            if (NULL == pNcpTaskSwState->pidArray[i])
            {
                 ...
                 * Add this process
                 */
                NCP_CALL(ncp_nvm_robust_malloc(sizeof(ncp_task_process_info_t), 
                     (void **)&pNcpTaskSwState->pidArray[i],
                     NCP_DO_MEMSET));
                       ...
                       ncp_task_resource_name_t *threadName)
{
    ...
    if (0 == pvtTqsHdl->pProcess->numThreads)
    {
        ...
        NCP_CALL(ncp_nvm_robust_free((void **)&(pNcpTaskSwState->pidArray[pvtTqsHdl->pProcess->pid])));   /* free process struct */
        pNcpTaskSwState->numActivePids--;
    }
        ...
        (pvtTqsHdl->completionsIssued[NCP_TASK_TXQ_0] + pvtTqsHdl->completionsIssued[NCP_TASK_TXQ_1]))
    {
        ...
        if (!inRecovery)
        {
            ...
            NCP_TASKIO_TRACEPOINT(Intel_AXXIA_ncp_nca,
                                  ...
                                  NCP_MSG_ERROR,
                                  "Attempt to unbind with %d task completions owed, %d issued on TXQ0, %d issued on TXQ1\r\n",
    ...
    int    tqsId)
{
    ...
    /* IPCQ */
    NCP_CALL(ncp_ncav3_get_pcq_info(
                ncp,
                tqsId,
                IPCQ,
                NCP_TASK_UNUSED_PARAM,      /* opcqIndex */
                NCP_TASK_UNUSED_PARAM,      /* poolId */
                NCP_TASK_UNUSED_PARAM,      /* bufSizeBits */
                &pTqs->rxQ));
    ...
    pTqs->txQ0.nEntriesMinusOne = pTqs->txQ0.nEntries - 1;
    pTqs->txQ0.nEntriesMinusTwo = pTqs->txQ0.nEntries - 2;    
    pTqs->txQ0.nEntriesDiv2     = pTqs->txQ0.nEntries / 2;        
    pTqs->txQ0.cookie = NCP_TASK_PCQ_COOKIE; 
    pTqs->txQ0.u.opcq_info.pOPCQentry = pTqs->txQ0.pPCQbase; 
    pTqs->txQ0.u.opcq_info.pPrefetchEntry = pTqs->txQ0.u.opcq_info.pOPCQentry + 1;
    pTqs->txQ0.u.opcq_info.pLastEntry = pTqs->txQ0.u.opcq_info.pOPCQentry + pTqs->txQ0.nEntries - 1;           
    pTqs->txQ0.u.opcq_info.relId = NCP_TASK_TXQ_0;  
    pTqs->txQ0.hwProducerVal = 0;
    pTqs->txQ0.hwConsumerVal = 0;
    pTqs->txQ0.toggleBit = 0;
    if (pTqs->shareCtl.shareFlags.sharedTxQueue0)
    {
        pTqs->txQ0.shared = TRUE;
        NCP_CALL(ncp_task_v3_create_lock(&pTqs->txQ0.qLock, mtxAttr));
    ...
    /* Allocate aux data array for TXQ0 */
    /*TODO*/
    NCP_CALL(ncp_nvm_robust_malloc(
                pTqs->txQ0.nEntries * sizeof(ncp_task_per_oPCQ_aux_data_t), 
                (void **)&pTqs->txQ0.u.opcq_info.pBaseAuxEntry,
                NCP_DO_MEMSET));
    /* init aux data */    
    pTqs->txQ0.u.opcq_info.pAuxWriteEntry 
        = pTqs->txQ0.u.opcq_info.pAuxReadEntry      
        = pTqs->txQ0.u.opcq_info.pBaseAuxEntry;
    pTqs->txQ0.u.opcq_info.pLastAuxEntry  
        = pTqs->txQ0.u.opcq_info.pBaseAuxEntry + pTqs->txQ0.nEntries;   
    ...
    pTqs->txQ1.nEntriesMinusOne = pTqs->txQ1.nEntries - 1;
    pTqs->txQ1.nEntriesMinusTwo = pTqs->txQ1.nEntries - 2;    
    pTqs->txQ1.nEntriesDiv2     = pTqs->txQ1.nEntries / 2;         
    pTqs->txQ1.cookie = NCP_TASK_PCQ_COOKIE;
    pTqs->txQ1.u.opcq_info.pOPCQentry = pTqs->txQ1.pPCQbase;
    pTqs->txQ1.u.opcq_info.pPrefetchEntry = pTqs->txQ1.u.opcq_info.pOPCQentry + 1;
    pTqs->txQ1.u.opcq_info.pLastEntry = pTqs->txQ1.u.opcq_info.pOPCQentry + pTqs->txQ1.nEntries - 1;                        
    pTqs->txQ1.u.opcq_info.relId = NCP_TASK_TXQ_1;    
    pTqs->txQ1.hwProducerVal = 0;
    pTqs->txQ1.hwConsumerVal = 0;
    pTqs->txQ1.toggleBit = 0;
    if (pTqs->shareCtl.shareFlags.sharedTxQueue1)
    {
        pTqs->txQ1.shared = TRUE;
        NCP_CALL(ncp_task_v3_create_lock(&pTqs->txQ1.qLock, mtxAttr));
    ...
    /* Allocate aux data array for TXQ1 */        
    NCP_CALL(ncp_nvm_robust_malloc(
                pTqs->txQ1.nEntries * sizeof(ncp_task_per_oPCQ_aux_data_t), 
                (void **)&pTqs->txQ1.u.opcq_info.pBaseAuxEntry,
                NCP_DO_MEMSET));
    /* init aux data */    
    pTqs->txQ1.u.opcq_info.pAuxWriteEntry 
        = pTqs->txQ1.u.opcq_info.pAuxReadEntry      
        = pTqs->txQ1.u.opcq_info.pBaseAuxEntry;
    pTqs->txQ1.u.opcq_info.pLastAuxEntry  
        = pTqs->txQ1.u.opcq_info.pBaseAuxEntry + pTqs->txQ1.nEntries;   

    /* keep track of which pools used across all TQSets */ 
    pNcpTaskSwState->perDomainPoolsMask |= pTqs->validPoolsMask;

    pTqs->cpuPoolId = 0;

    if (pTqs->validPoolsMask & (1<<NCP_TASK_CPU_POOL_ALIAS))
    {
        for (poolId = NCP_NCAV3_FIRST_CPU_POOL_ID;
             poolId <= NCP_NCAV3_LAST_CPU_POOL_ID;
             poolId++)
        {
            if (1 == ((pTqs->validPoolsMask >> poolId) & 0x1))
            {
                pTqs->cpuPoolId = poolId;
                NCP_TASK_TBR_ADD_TO_CPUPOOL_TQS_LIST(poolId, pTqs);
                break;
            }
        }
    }        
    ...
    {    
                ...
                poolId++)
        {
            ...
            pAllocator = &pTqs->mmeAllocator[poolId];
            pAllocator->id = poolId;
            if (0 == (pTqs->validPoolsMask & (1<<poolId)))            
            {
                pAllocator->valid = FALSE;
            }  
            else
            {
                int sz;
                ...
                { 
                    ...
                    ncpStatus = ncp_ncav3_get_pcq_info(
                            ncp,
                            tqsId,
                            MPCQ,
                            0,                      /* opcqIndex */
                    ...
                    if (NCP_ST_SUCCESS == ncpStatus)
                    {
                        ...
                        p_mPCQ->u.mpcq_info.pPrefetchEntry
                            = p_mPCQ->u.mpcq_info.pMPCQentry + 1;
                        p_mPCQ->u.mpcq_info.pLastEntry 
                            = p_mPCQ->u.mpcq_info.pMPCQentry + p_mPCQ->nEntries - 1; 
                        p_mPCQ->u.mpcq_info.numAvail = 0;
    ...
    /* Init general tqs state members */
...
NCP_RETURN_LABEL
    ...
    if (ncpStatus != NCP_ST_SUCCESS)
    {
        ncp_ncav3_free_tqs_state(tqsId);
    }
...
NCP_RETURN_LABEL
    ...
    {    
        ...
        NCP_TASKIO_TRACEPOINT(Intel_AXXIA_ncp_nca,
                              ...
                              NCP_MSG_DEBUG,
                              "In free: TxQ_%d completions issued incremented by %d, total = %d\r\n",
                              p_oPCQ->u.opcq_info.relId,
                              numCompleted,
