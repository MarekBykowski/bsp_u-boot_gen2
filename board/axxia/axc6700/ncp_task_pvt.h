...
// LAPAJ #include "ncp_sal.h"
// LAPAJ #include "ncp.h"
// LAPAJ #include "ncp_pvt.h"
#include "ncp_task.h"

//#include "ncp_sal_memory.h"
//#include "ncp_sal_barrier.h"
//#include "ncp_sal_timer.h"

#include "ncp_ncav3.h"

#if !defined(NCP_KERNEL) && defined(NCP_NCA_ARM_PMU_PROFILING_ENABLED)
#include "ncp_sal_pmu.h"
#endif
...
#undef  ARCH_HAS_STORE_RELEASE_INSTRUCTION /* prevent redefine compile err */
#define ARCH_HAS_STORE_RELEASE_INSTRUCTION 0 /* need 8bit version,   then this can be reenabled */
...
typedef struct ncp_task_pvt_tqsHdl_data_s 
{
  ...
  ncp_uint32_t             completionsOwed;      /* used for manual completion mode */
  ncp_uint32_t             completionsIssued[NCP_NCAV3_NUM_OPCQS_PER_TQS]; /* used for manual completion mode */
...
#define NCP_NCA_CRITICAL_SECTION_FLAG_DECL(_critSecFlag) ncp_bool_t _critSecFlag=FALSE; if (_critSecFlag){};
...
//LAPAJ#define NCP_VALIDATE_NCP_HDL(ncpHdl)     ncp_validate_handle(ncpHdl)
//LAPAJ#define NCP_VP_VALIDATE_HANDLE(vpHdl)        ncp_vp_validate_handle(vpHdl)
#else
//LAPAJ#define NCP_VALIDATE_NCP_HDL(ncpHdl)   NCP_ST_SUCCESS
//LAPAJ#define NCP_VP_VALIDATE_HANDLE(vpHdl)  NCP_ST_SUCCESS
#endif /* NCP_KERNEL */
